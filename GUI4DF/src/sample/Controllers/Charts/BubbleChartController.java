...
public class BubbleChartController {
    ...
    public BubbleChartController(ComboBox<String> cmbColNamesX,ComboBox<String> cmbColNamesY,ComboBox<Integer> additionalCountCmb,ScrollPane chartPane,ComboBox<String> inaCaseOfTwoCmb,
    ...
    public void showAlert(String mes) {
        ...
        alert.setHeaderText("Bubble Chart Drawer");
        alert.setContentText(mes);
        // alert.initModality(Modality.APPLICATION_MODAL);
        alert.showAndWait();
    }

    public void drawBubbleChart(DataFrame df) {
        //defining the axes
        ...
        if (df.get(colNameX).zwrocObiekt(0) instanceof StringValue || df.get(colNameX).zwrocObiekt(0) instanceof DateTimeValue || df.get(colNameY).zwrocObiekt(0) instanceof StringValue || df.get(colNameY).zwrocObiekt(0) instanceof DateTimeValue) {
            ...
            if (additionalCountCmb.getSelectionModel().getSelectedIndex() == 0) {
                ...
                bubbleChart.setTitle("Wykres zależności " + colNameY + " od " + colNameX);
                //defining a series
                ...
                chartPane.setContent(bubbleChart);
                ...
                if (df.get(colNameY2).zwrocObiekt(0) instanceof StringValue || df.get(colNameY2).zwrocObiekt(0) instanceof DateTimeValue) {
                    showAlert("Wartości w wybranych kolumnach muszą być numeryczne!");}
                        ...
                        bubbleChart.getData().addAll(series,series2);
                ...
                if (df.get(colNameY2).zwrocObiekt(0) instanceof StringValue || df.get(colNameY2).zwrocObiekt(0) instanceof DateTimeValue||
                    ...
                    bubbleChart.getData().addAll(series,series2,series3);
